...
function compose_directory(uuid, target, data, isDirectoryInFolder){
	...
	if(isDirectoryInFolder){
		target = (target == "ggv") ? target.toUpperCase() : target.charAt(0).toUpperCase() + target.slice(1);
	...
	$.ajax({
		...
		success:function(json){
			...
			$.each(json.list, function(index, file){
				var li_header = "<li id='" + file.name.toLowerCase() + "' class='file'>";
				if(file.type == "폴더"){ li_header = "<li id='" + file.name.toLowerCase() + "' class='file folder'>"; }
				...
				if(file.link != undefined){
					$(".file#" + file.name.toLowerCase()).attr((isPC()) ? "ondblclick" : "onclick", "window.open('" + file.link + "')");
				}
			});
		}
	});
